...
class BoundaryCondition:
    ...
    # Static methods ---------------------------------------------------------------------------------------------------
    @staticmethod
    def __check_BCpara(BCnode: list[dict], BCpara: list[dict], freq_max: float):
        """Check if BCpara is compatible with AcousticsSimulation.BCnode and satisfies physical admissibility condition.
        ...
        for index, paras in enumerate(BCpara):
            ...
            assert numpy.abs(BoundaryCondition.__compute_Re(omega, paras)).all() <= 1.0, "[edg_acoustics.BoundaryCondition] All reflection coefficient must be smaller than 1"
    ...
    @staticmethod
    def __init_ADEvariables(BCpara: list[dict], BCnode: list[dict]):
